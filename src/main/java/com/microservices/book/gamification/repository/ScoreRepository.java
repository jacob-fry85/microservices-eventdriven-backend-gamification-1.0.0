...
public interface ScoreRepository extends CrudRepository<ScoreCard, Long> {
    ...
    /**
     * Retrieves a alist of {@link LeaderBoardRow}s representing the Leader Board
     * of users and their total score
     *
     * @return the leader board, sorted by highest score first
     */
    @Query("SELECT NEW com.microservices.book.gamification.game.domain.LeaderBoardRow(s.userId, SUM(s.score)) " +
        "FROM ScoreCard s " +
